...
describe('Testing adminQuizQuestionMove function:', () => {
  const questionBody1 = {
    question: 'who up lol',
    duration: 5,
    points: 5,
...
describe('Testing adminQuizQuestionMoveV2 function:', () => {
  const questionBody1 = {
    question: 'who up lol',
    duration: 5,
    points: 5,
    ...
    thumbnailUrl: 'https://example.com/path/image/bruh.png',
  };
  const questionBody2 = {
    question: 'but seriously who is up',
    duration: 5,
  ...
  beforeEach(() => {
    quiz = quizCreateRequestV2(john.token, 'boring quiz', 'loveless and lifeless');
    question1 = questionCreateRequestV2(john.token, quiz.quizId, questionBody1);
    question2 = questionCreateRequestV2(john.token, quiz.quizId, questionBody2);
    question3 = questionCreateRequestV2(john.token, quiz.quizId, questionBody3);
  });
  describe('Expected behaviour under success(V2)', () => {
    test('Moves questions around nicely', () => {
      questionMoveRequestV2(john.token, quiz.quizId, question2.questionId, 0);
      const quizInfo = quizInfoRequestV2(john.token, quiz.quizId);
      expect(quizInfo.questions[0].questionId).toEqual(question2.questionId);
      questionMoveRequestV2(john.token, quiz.quizId, question1.questionId, 0);
      const quizInfo2 = quizInfoRequestV2(john.token, quiz.quizId);
      expect(quizInfo2.questions[0].questionId).toEqual(question1.questionId);
    });
  });
  ...
  describe('Expected behaviour if requests are weird(V2)', () => {
    test('invalid token should result in error', () => {
      ...
      const result = questionMoveRequestV2(john.token + '1', quiz.quizId, question2.questionId, 0);
      const idAfter = quizInfoRequestV2(john.token, quiz.quizId).questions[0].questionId;
      expect(idBefore).toEqual(idAfter);
      expect(result).toStrictEqual(ERROR);
    });
    test('invalid wanted place should result in error', () => {
      const quizInfoPrior = quizInfoRequestV2(john.token, quiz.quizId);
      const result1 = questionMoveRequestV2(john.token, quiz.quizId, question2.questionId, -1);
      const result2 = questionMoveRequestV2(john.token, quiz.quizId, question2.questionId, 3);
      expect(result1).toStrictEqual(ERROR);
      expect(result2).toStrictEqual(ERROR);
      expect(quizInfoRequestV2(john.token, quiz.quizId)).toStrictEqual(quizInfoPrior);
    });
    test('invalid quizId should result in error', () => {
      const result = questionMoveRequestV2(john.token, quiz.quizId + 1, question2.questionId, 0);
      expect(result).toStrictEqual(ERROR);
    });
    test('invalid questionId should result in error', () => {
      const result = questionMoveRequestV2(john.token, quiz.quizId,
        Math.max(question2.questionId, question1.questionId, question3.questionId) + 1, 0);
      expect(result).toStrictEqual(ERROR);
    });
  });
