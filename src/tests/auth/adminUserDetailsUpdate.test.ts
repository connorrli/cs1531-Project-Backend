...
describe('Testing adminUserDetailsUpdateV2 function:', () => {
  ...
  test.each([
    ...
    expect(userDetailsUpdateRequestV2(user1Token, email, nameFirst, nameLast)).toStrictEqual(expectedReturn);
  });
  test('Testing Invalid UserId', () => {
    expect(userDetailsUpdateRequestV2(user3Token, 'z000000@ad.unsw.edu.au', 'John', 'Doe')).toStrictEqual(ERROR);
  });
});
