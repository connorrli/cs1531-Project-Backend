...
describe('Testing adminUserPasswordUpdateV2 function:', () => {
  const INIT_VALID_EMAIL = 'z000000@ad.unsw.edu.au';
  const INIT_VALID_PASSWORD = 'Password123';
  let userToken1 : string;
  let userToken2 : string;
  ...
  test('User Doesn\'t Exist', () => {
    const newPassword = 'password321';
    const response = userPasswordUpdateRequestV2(userToken2, INIT_VALID_PASSWORD, newPassword);
    expect(response).toStrictEqual(ERROR);
  });
  test('New Password is a Previous Password', () => {
    const newPassword = 'Password12345';
    userPasswordUpdateRequestV2(userToken1, INIT_VALID_PASSWORD, newPassword);

    const response1 = userPasswordUpdateRequestV2(userToken1, newPassword, INIT_VALID_PASSWORD);
    expect(response1).toStrictEqual(ERROR);
