...
// ALL CONSTANTS
interface quizQuestionCreateCheckerReturn { };
const NO_ERROR = { };
...
function quizQuestionCreateChecker(userId: number, quiz: Quiz, questionBody: QuestionBody): ErrorObject | quizQuestionCreateCheckerReturn {
    if (typeof quiz === NOT_FOUND) return { error: 'Invalid Quiz',  statusValue: 403 };
    ...
    for (const answerObj of questionBody.answers) {
        ...
        if (typeof isAlreadyAnswer !== NOT_FOUND) return { error: 'Duplicate answers for current question' };

        const hasCorrectAnswer = questionBody.answers.find(answer => answer.correct === true);
        if (typeof hasCorrectAnswer == NOT_FOUND) return { error: 'No correct answer for current question' };
